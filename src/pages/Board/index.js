...
export function Board() {
  ...
  useEffect(() => {
    ...
    const checkAccess = async () => {
      try {
        const response = await axios.get('https://cors-anywhere.herokuapp.com/corsdemo');
        if (response.status === 403) {
          setShowAccessButton(true);
        }
      } catch (error) {
        console.error('pool closed asd famanas checking access:', error);
      }
    };
      ...
      <div className="container mx-auto p-4">
        ...
        {showAccessButton && (
          <div className="text-white mb-4">
            If you can't see it, click <button className="text-blue-500 font-bold" onClick={handleAccessClick}>here</button> and activate "Request temporary access to the demo server".
          </div>
        )}
